...
function extractNameFromTemplate(value) {
    return value.slice(0, -1).slice(7);
}
...
function getRectangleString(width, height) {
    if (width < 2 || height < 2)
        return "Cannot create rectangle.";
    else 
...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < str.length; i++){
        if ((index = fromLine.indexOf(str[i])) >= 0)
            answer += toLine[index];
        else    
            answer += str[i];
    }
...
function isString(value) {
    return !!value ? (typeof value == 'string') || (value.constructor.name == "String") : false;
} 
